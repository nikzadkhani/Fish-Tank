...
public class Component implements Rotatable, Nameable, Colorable, UpdatingDisplayable {
	...
	private Point3D previousRotation;
...
//		gl.glMultMatrixf(rotationQuaternion.to_matrix(), 1);
	...
	public void setRotationQuaternion(Quaternion newQuaternion) {
		this.rotationQuaternion = newQuaternion;
	}
...
//			System.out.println("hit wall");
	...
	public void faceTowardsNewDirection() {
		double newAngleX = Math.toDegrees(Math.atan(deltaY/deltaZ));
		double newAngleY = Math.toDegrees(Math.atan(deltaX/deltaZ));
		double newAngleZ = Math.toDegrees(Math.atan(deltaY/deltaX));

		if (deltaZ == 0) {
			newAngleX = this.xAngle();
			newAngleY = this.yAngle();
		}
		if (deltaX == 0) {
			newAngleZ = this.zAngle();
		}
		this.setAngles(newAngleX,newAngleY,newAngleZ);
	}
